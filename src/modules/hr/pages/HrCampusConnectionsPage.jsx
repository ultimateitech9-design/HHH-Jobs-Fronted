...
function StatPill({ label, value, tone, onClick }) {
  ...
  return (
    <Component
      ...
      className={`inline-flex h-8 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 text-[11px] font-semibold leading-none text-slate-600 shadow-[0_1px_0_rgba(15,23,42,0.02)] transition ${onClick ? 'hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900' : ''}`}
    >
      <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${dotColors[tone] || dotColors.slate}`} />
      <span>{label}</span>
