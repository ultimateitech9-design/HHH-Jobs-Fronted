      ...
      {(eyebrow || title || subtitle || action) ? (
        <div className="mb-3.5 flex min-w-0 flex-col gap-2 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            {eyebrow ? (
              <p className="mb-1 text-[10px] font-black uppercase tracking-[0.16em] text-brand-700">{eyebrow}</p>
            ) : null}
            {title ? <h2 className="break-words font-heading text-[1.12rem] font-bold leading-tight text-navy md:text-[1.28rem]">{title}</h2> : null}
            {subtitle ? <p className="mt-1 max-w-3xl text-[12.5px] leading-4.5 text-slate-500">{subtitle}</p> : null}
          </div>
